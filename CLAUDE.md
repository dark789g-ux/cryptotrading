...
- 启动阻塞变量见 [.env.example](.env.example) 中标注 `[REQ]` 的项（`TUSHARE_TOKEN`、`LLM_API_KEY` / `LLM_BASE_URL` / `LLM_PROVIDER` / `LLM_MODEL`）。`DailyReviewModule` 默认装载，缺 LLM 配置会导致后端启动失败。量化 SSE 用 `QUANT_SSE_TOKEN_SECRET`（非 `[REQ]`，有 `JWT_SECRET` 兜底链，见 `.env.example`）。
