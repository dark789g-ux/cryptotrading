...
| # | 任务 | 文件域 | 估时 |
|---|---|---|---|
| 1 | `strategy/exit_rules.py` 出场规则模块 + 单测 4 路径 | `apps/quant-pipeline/src/quant_pipeline/strategy/` + `tests/unit/` | 2 天 |
| 2 | `labels/strategy_aware.py` + 5 个坑处理 + 写 `factors.labels` | `apps/quant-pipeline/src/quant_pipeline/labels/` | 3 天 |
| 3 | `features/` 中性化 + 标准化 + 写 `factors.feature_matrix` | `apps/quant-pipeline/src/quant_pipeline/features/` | 2 天 |
| 4 | `training/lightgbm_lambdarank.py` 单 fold 训练 + artifact 落盘 | `apps/quant-pipeline/src/quant_pipeline/training/` | 3 天 |
| 5 | `inference/runner.py` + score_writer + 推理前必检集成 | `apps/quant-pipeline/src/quant_pipeline/inference/`、`quality/` | 2 天 |
| 6 | worker dispatcher 覆盖 train / infer + heartbeat + cancel | `apps/quant-pipeline/src/quant_pipeline/worker/` | 2 天 |
