...
ml.jobs (
  id              uuid PK,
  run_type        text NOT NULL,  -- sync|quality|factors|labels|features|train|infer|optuna|seed_avg|noop
...
CREATE INDEX ON ml.jobs (status, created_at);
...
CREATE INDEX ON ml.scores_daily (trade_date, model_version, rank);

ml.quality_reports (
  id              bigserial PK,
  trade_date      char(8) NOT NULL,
  level           text NOT NULL,    -- info|warn|critical
  rule            text NOT NULL,    -- 'pit_finance' / 'adj_jump' / 'row_count_drift' / ...
...
### 4.5 迁移执行序列（M0 必须严格按此顺序）

1. PG 创建 `raw / factors / ml` 三 schema（IF NOT EXISTS）。
2. 执行手写 SQL migration（NestJS 侧 / 含 docker exec 脚本 / 含回滚脚本），形如：
   ```sql
   ALTER TABLE public.a_share_daily_quote SET SCHEMA raw;
   ALTER TABLE raw.a_share_daily_quote RENAME TO daily_quote;
   -- ...对每张表重复
   ```
3. NestJS entity 全部改 `@Entity({ schema: 'raw', name: 'daily_quote' })` 并去 `a_share_` 前缀；`pnpm --filter @cryptotrading/server build` 必须通过。
4. NestJS 侧重启，跑一次既有 A 股同步任务，验证写 raw.* 成功。
5. `uv run quant worker run` 启动 Python worker，插入一条 `ml.jobs(run_type='noop')` 验证消费链路。

**回滚脚本**（必须随 migration 一起提供）：
```sql
ALTER SCHEMA raw RENAME TO raw_backup;
CREATE SCHEMA raw;
-- 必要时把 raw_backup.* 复原到 public.a_share_*
```
...
│   │   ├─ daily.py / daily_basic.py / adj_factor.py /
│   │   │   stk_limit.py / suspend.py / index_member.py /
│   │   │   fina_indicator.py / trade_cal.py
│   │   └─ orchestrator.py
│   ├─ quality/
...
- `synchronize: false`；本 module 引入新表也走手写 SQL migration（与 §4.5 同批）
- 动态 SQL 构建禁止把前端字段名拼入：scores / runs 的 filter / sort 必须经过 `FIELD_COL_MAP` 翻译，未命中 `logger.warn` + skip

**进度推送方案**：
- Python worker 在更新 `ml.jobs.progress` 后立即 `NOTIFY ml_job_progress, '{"job_id":"...","progress":47}'`
...
**交付物**：
1. 手写 SQL migration（含 docker exec 脚本 + 回滚脚本）覆盖 §4.5 全部 5 步
...
- §4.5 中的"5 步发布序列说明书"作为 M0 README 一节落盘
...
6. NestJS 侧新增 `raw.stk_limit / raw.suspend_d / raw.index_classify / raw.index_member / raw.fina_indicator / raw.trade_cal` 同步逻辑
...
### 8.3 M2 · 标签 + LightGBM 训练 MVP 通路 + ml.jobs 骨架（3 周）

**目标**：从 `ml.jobs` 触发 → `model.txt` 落盘 → `ml.scores_daily` 写入的 end-to-end 跑通。**不追求模型质量**。
...
6. inference 写 `ml.scores_daily`（含 `model_version`）
7. NestJS `modules/quant/` jobs / scores / runs controller 上线（不接前端）
8. Python worker dispatcher 覆盖 `train` / `infer` 两个 `run_type`
9. 训练前自动跑 quality 门禁；失败 → `ml.jobs.status='blocked'` + `blocked_reason`

**验收门槛**：
- 一次 `POST /quant/jobs { run_type:"train", params:{...} }` → `ml.model_runs` 出现一行 → `ml.scores_daily` 当日 ≈ 5500 支股票全部有评分
...
### 8.4 M3 · Walk-Forward 评估 + 三组对照 + 前端评分看板（3 周）
...
5. Vue：`/quant` Overview + `/quant/scores` + `/quant/runs` 三页
6. NestJS scores / runs controller 接前端；模型版本切换器（query string）

**验收门槛**：
- 三组对照实验报告自动生成且可下载
- GBDT vs 线性 OOS NDCG@10 提升 ≥ 3%（doc/09 经验底线；不达不能进 M4，需排查标签 / 因子）
- Walk-Forward fold ≥ 6 折，每折 IC / NDCG / 扣成本年化均有记录
- 前端三页手测主流程无 5xx；列表分页、模型版本切换、Top-K 调整均工作
- 同一交易日 scores_daily 两个 `model_version` 共存查询无串扰
...
3. SSE 进度推送 + PG `LISTEN ml_job_progress` 通道
4. Optuna 调参（doc/05 四个主旋钮：`num_leaves` / `min_data_in_leaf` / `feature_fraction` / `learning_rate`）作为独立 `run_type='optuna'`
5. Seed Averaging（5 seed 平均）作为 `run_type='seed_avg'`
6. 监控：**每日推理后**自动算 IC / 评分分布 / 与上次的特征漂移 PSI，超阈值落 `ml.quality_reports(level=warn|critical)`
7. cron / Windows 任务计划脚本模板：每日盘后 18:00 触发 `sync → quality → infer` 链
8. 复盘报告：本机 CPU 跑当前规模因子 / 模型 / 调参的时延实测；判断是否需扩容（仅给结论，不做实现）

**验收门槛**：
- 在 web 上点"触发训练" → 进度条实时滚动（< 2 秒延迟）→ 完成后 RunDetail 显示 SHAP
- Optuna 一次完整调参（≥ 50 trial）能跑完且 best_trial 落 `ml.model_runs`
- 模拟一次特征漂移（人为篡改输入分布）→ `ml.quality_reports` 产生 `critical` 行 → 前端 Overview 顶部告警条
- 任务计划脚本在本地连续运行 3 个交易日无人值守，每日 ml.scores_daily 当日 100% 完整
- 单文件 ≤ 500 行硬约束在所有新增 Vue 文件上通过（`QuantRunDetailView` 需提前拆分）
...
4. **数据迁移期 NestJS 与 Python 的发布顺序**
   必须 NestJS entity 切到 `raw` 之后立即跑 migration，再启动 Python sync。中间窗口内既有 NestJS 同步会失败。**M0 验收要求附 "5 步发布序列说明书" 作为 README 一节**（§4.5 已给出原型）。
...
7. **PG LISTEN/NOTIFY 跨进程注意事项**
   NestJS 的 SSE 桥接进程必须维持一条独立、长生命周期的 PG 连接专门用于 `LISTEN`；不可与请求处理池复用。重连后需重新 `LISTEN`。订阅者断开时记得清理。

8. **Optuna trial 中断恢复**
   Optuna 50 trial 数小时级，过夜跑期间机器重启会导致进度丢失。**M4 选型时优先选 Optuna RDB storage（PG）而非默认 in-memory**，使 trial 可恢复。
...
- `CLAUDE.md` 项目硬约束（编码 / NestJS 规范 / Vue 规范 / 时间规范 / 第三方 API 规范）← §4.5 / §7 / §9 的合规来源
