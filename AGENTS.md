...
- 书写 PLAN 时，需要把 SubAgent 编排写进去，尽可能的派发 SubAgent 执行方案，执行完成后需要再派发 SubAgent 进行代码审查，确保代码质量。

## Agent 派发规范

- **派发 Agent 工具时不使用 `isolation: "worktree"`**，agent 直接在当前会话工作目录（主仓库）中运行。
- 文件变更直接落在主仓库，避免产生需要手动同步/合并的隔离 worktree。
- 如果用户明确要求 worktree 隔离，再传入 `isolation: "worktree"`。
