...
| 5 | 条件系统 operator 是否含 `lte` | 象限边界（`amv_macd <= 0`）真机表达 | grep operator 白名单；若仅 `lt`，离线 CASE 与真机条件统一改用严格 `<`，`=0` 归负侧并记录口径 |
...
2. **诚实边界声明**（映射表附录）：哪些 edge 稳健（跨年一致）、哪些靠特定行情
   （如 Q1×2025）；四象限轴本身未经 holdout 保护这一事实必须写明；Q4×2022 正
   收益反例必须标注。
...
M0 核查点清扫（1/2/5/6 必须先于建 run；3 随首 run；4 随收口）
  ▼
M1 count 预检 + 18 个宽锚点 run 分批建跑（核查点 3 定排程）
  ▼
