...
- 单族全窗口 > 100 万 → 按序收紧：族 A 先 delta 回 0.66、再补 `ma30>ma60`；
  族 B 按 J<10 → J<0 → J<-5（梯子只取离线扫描档位集中的值，被收掉的宽档如需
  考察须单独建 run）。收到 ≤100 万为止，收紧动作记入研究日志。
- 同时执行 04 文档核查点 2（大样本聚合栈溢出是否已修）后才允许建首个 run。

### 第一轮出场配置（每个 × 每族 = 一个 run，第一轮共 18 个；核查点 1 已核：支持）

| # | exit_mode | 参数 |
|---|---|---|
| 1-3 | `trailing_lock` | max_hold ∈ {无, 10, 20} |
| 4-6 | `fixed_n` | N ∈ {5, 10, 20} |
| 7 | `strategy` | 卖出：KDJ 超买离场（`kdj_j gt 90`，阈值第一轮固定，其它档位属第二轮），max_hold=20 兜底 |
| 8 | `strategy` | 卖出：跌破 BBI 多空线（`close lt bbi`，field 比较），max_hold=20 兜底。原设想 ma10 不在字段映射（仅 ma5/30/60/120/240），BBI 速度最接近，2026-06-11 落源头核实后替换 |
| 9 | `strategy` | 卖出：大盘 regime 恶化离场（`oamv_macd lt 0`），max_hold=20 兜底 |

- 配置 9 可行性已核（核查点 1，2026-06-11）：strategy 出场与买入条件同走
  `buildAShareQuery`，大盘字段出场条件原生支持；M1 建首个此类 run 前以小
  窗口 smoke 复验一次。
- 第一个 run 实测耗时后再定其余 17 个的排程（串行/并行），见核查点 3。
- 第二轮按第一轮结果可选加 run（新出场假设或被收紧掉的宽阈值档），总数不设
  上限但每个新 run 须在研究日志写明动机。**第二轮 run 仅服务 train 窗探索**；
  holdout 评估额度是跨所有轮次的全局一次性预算（见双保险协议），不因加 run
  而增加。
...
4. **样本地板**：train 窗 n ≥ 500；holdout 窗**信号数** ≥ 200——直接 count
   holdout 窗内该配置的信号行数（**仅 count、不读 ret**，计数不构成对收益的
   偷看，且比按 train 密度折算准确——各象限信号占比年际漂移剧烈）。不达标
   组合直接淘汰，不得进入预登记。

## 双保险验证协议（硬纪律）

- **窗口**：train = 2022.4–2024.12；holdout = 2025.1–2026.5。
- **诚实声明：holdout 在本研究中是弱保护（degraded）**——①四象限轴本身凭
  全周期数据选定；②预验表（01）已暴露各象限在 2025–2026 的 kelly；③入场族
  A（搬砖）的身份核心也由含 holdout 的全周期表现演化而来。holdout 保护的
  只是「象限内变体选择」这一层，且研究者已知 holdout 期行情风格，选过滤器
  时可能不自觉偏向"2025 风格"候选。此事实必须原样写进终态边界声明；真正
  零接触的验证是影子期（见本节末）。
...
- **单次评估 + 全局预算**：holdout 每配置只评一次，结果无论好坏照实记入
  研究日志，**禁止评完回头换候选再评**。holdout 评估额度为**跨所有轮次的
  全局一次性预算 12 次（4 象限 × 3）**——第二轮新增 run/候选不重置预算；
  额度耗尽后 holdout 永久封闭，其后任何新候选只能以 train+按年口径报告并
  显式标注「未经 holdout」，或留待影子期/新数据形成的验证窗。
- **达标线**（全部满足才算"该象限有可用策略"，规则在预登记时即锁死，
  不得在看到 holdout 结果后调整）：
  1. holdout kelly ≥ 0.02，且 ≥ 同象限基线（搬砖7条+trailing_lock）的
     holdout kelly + 0.03（即「优于基线」的量化定义；空仓基线 kelly=0，
     由绝对下限 0.02 覆盖）；
  2. 全周期按年分解 ≥4/5 年非深亏（深亏 = 单年 kelly < -0.05）**且**
     ≥3/5 年为正；
  3. **去单年依赖检验**：剔除表现最佳的单个年份后，全期 kelly 仍 > 0
     （专防"多年浅亏 + 单年暴利"形态——01 实测的 Q1 即此反面教材，深亏
     豁免条款单独使用时会原样放行它）；
  4. holdout 实际样本 n ≥ 200。
  年份计数规则：单年样本 n < 200 的年份格不参与按年计数（既不算正也不算
  亏，2026 仅 5 个月常落此类），照实标注。
- **基线对照**：终表每象限必含三行——「搬砖7条+trailing_lock 基线」「空仓
  （kelly 记 0）」「调优胜者」。胜者以达标线条款 1-4 为准；不达标 → 该象限
  结论降级为「空仓」或「仅基线」，照实写入映射表。
- **真机收口**：每象限胜者建 signal_test（入场 = 胜者条件 + 该象限两条 oamv
  条件）→ run → `sample_count`/`kelly_f` 与离线**逐位对账**；差异须可解释
  （如停牌/次新过滤），解释不了视为 bug，修完重对。
- **影子期终验（唯一零接触数据）**：config v1 激活后，以 2026-06 之后的
  Phase 2 每日流水线做 walk-forward 影子验证——运行 ≥8 周后复核各象限实际
  信号的前向表现，作为配置保留/降级依据。该窗口在本 spec 撰写时点尚不存在，
  是唯一完全未被本研究燃烧的验证数据。
...
WITH t AS (
  ...
  WHERE tr.run_id = '<anchor-run-id>'
    AND o.amv_dif IS NOT NULL AND o.amv_macd IS NOT NULL
    -- NULL 行排除（而非被 ELSE 吞进 Q4），与 Phase 2 classifyRegime 的
    -- null→unknown fail-closed 同口径；排除行数须记入研究日志
)
...
- `runs-manifest.md` —— 锚点 run 台账：族、出场配置、test id、run id、
  建跑日期、count 预检值、窗口。**离线切片 SQL 的 run_id 一律从台账复制**，
  混淆一个 id 整条分析就错；
- `sql/` —— 全部对账/分桶 SQL 脚本（可复跑）；
- 最终映射表与边界声明（交付物定义见 [04](./04-verification-and-risks.md#交付物与达标线)）。

锚点 run 的 signal_test 命名规范：`0amv-anchor-{A|B}-exit{1..9}`
（如 `0amv-anchor-B-exit3` = 超跌族 × trailing_lock mh=20），第二轮新增
出场配置顺延编号并在台账登记动机。
