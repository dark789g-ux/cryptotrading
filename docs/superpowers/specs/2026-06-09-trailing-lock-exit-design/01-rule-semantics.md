...
每个交易日 `bars[i]`，**跳过停牌日**（`continue`，不计 / 不触发 / 不更新 / 不动 prev_ma5）：

```text
stop_eff = stop_next                 # 今日生效的止损 = 昨日收盘设定的那个

── (0) 顺延中？ pending ≠ None:
        若 非封死跌停(raw_high > down_limit) → 出场 @adj_open, reason=pending, 结束
        否则 → 仍封死，continue（继续顺延）

── (1) 日内止损： stop_eff 非空 且 adj_low ≤ stop_eff ?
        若 封死跌停(raw_high ≤ down_limit) → pending='stop'，continue（卖不出，顺延）
        否则 → 出场 @min(stop_eff, adj_open), reason='stop', 结束
...
── (2) 收盘处理（未被止损）：
     ...
     (2b) 已锁定（含本日刚锁定）→ MA5 收盘离场：
             若 ma5 非空 且 prev_ma5 非空 且 adj_close < ma5 且 ma5 < prev_ma5 :
                 若 封死跌停 → pending='ma5_exit'，跳到 (2d)（顺延）
                 否则 → 出场 @adj_close, reason='ma5_exit', 结束
     (2c) 未锁定 → 更新"次日生效"止损 stop_next：
             方案二: 若 adj_close > cost → floor_active = True（sticky，一旦置真永久保持）
                     floor_active → stop_next = max(floor2(adj_low×0.999), floor2(cost×0.999))
                     否则         → stop_next = floor2(adj_low × 0.999)
             方案一: stop_next = floor2(adj_low × 0.999)
     (2d) （可选）max_hold 兜底：hold 达到 max_hold → 出场 @adj_close, reason='max_hold', 结束
     prev_ma5 = ma5
...
- **MA5 预热**：MA5 = 5 日复权收盘均值，持仓首日 T+1 需 T-3..T+1 的收盘价。数据窗口须**左扩 ≥4 个交易日**；
  预热不足时 `ma5=None`，(2b) 因 `ma5 非空` 守卫自然跳过 MA5 离场（止损逻辑不受影响）。
