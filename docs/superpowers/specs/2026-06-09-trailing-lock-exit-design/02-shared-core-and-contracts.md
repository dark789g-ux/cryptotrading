...
未列 `raw_*/limit` 的样例默认不触发限停板；**依赖锁定的样例须显式给 `signal_high`**；未给 `max_hold` 默认 None。
`exit_index` 为命中出场那根在 `bars` 中的下标（持仓首日 = bars[0]）；`hold_days` 为已走过可交易持有日数（buy=0）。

| # | 场景 | 关键输入(T+1 起) | 期望输出 |
|---|---|---|---|
| S1 | 方案一·跟踪止损出场 | signal_high=10.00；T+1(o10,l9.8,c10.2)；T+2(l10.5,h10.6,c10.5)锁定；T+3(l10.40) | exit@min(10.48,open3), reason=stop, scheme=1, exit_index=2, hold_days=2 |
...
| S11 | max_hold 兜底(可选) | max_hold=10；signal_high 设高位不锁定；全程不触发止损（无停牌） | exit@adj_close, reason=max_hold, exit_index=10, hold_days=10 |
| S12 | 窗口耗尽未出场 | 无 max_hold，窗口短，未触发任何条件 | no_exit（调用方收口） |
| S13 | 方案二·保本地板锁定当日首次浮盈 | 方案二；某锁定日同时 adj_low>signal_high 且 adj_close>cost（首次） | 冻结止损 = max(floor2(low×0.999), floor2(cost×0.999))，scheme=2（验 P3 裁决：(2-pre) 先激活地板） |
