...
3. 汇总落库：每只 `us_daily_quote` 覆盖 `[20250101,20260612]`、qfq/指标非空；`failed_items` 如实列出（实测 **0 项**；`SPCX` 仅 1 行但属正常——2026-06-12 IPO=窗口末日，非 failed_item），逐条报告，不静默。
...
- **`SPCX` = 真 SpaceX**：2026-06-12 Nasdaq IPO（=窗口末日），实测落 1 行属正常、非 failed_item；保留，end_date 推近重灌补后续交易日。
