...
2. **`compute_strategy_aware_labels`（`strategy_aware.py:360-371`）构造 `cand` 时派生
   ...
   - `end_padded` 缓冲（见 [03 §item-5](./03-exit-rules-fixes.md#item-5)）保证 `[start,end]`
     内的信号日总能取到 T+1。
   - `cand` 列变为 `[ts_code, signal_date, buy_date]` —— **`entry_date` 列被改名**。
   - ⚠️ **连带改动**：`strategy_aware.py:402-404` 现有
     `cand_dedup = cand.drop_duplicates(subset=["ts_code","entry_date"], keep="first")`
     的 `subset` 引用了将被改名的 `entry_date` 列，不改会 `KeyError`。`subset` 改为
     `["ts_code","signal_date"]`（按信号日去重：同一票同一信号日只保留一条候选）。
...
**`down_revision` 必须指向当前 alembic head**（实现时用 `alembic heads` 确认，写作本
spec 时 `versions/` 下最新为 `20260520_0001_factors_monthly_partitions.py`，勿误接到
`20260517_0001` 造成多头分支）。对 `factors.labels` 加列注释：
