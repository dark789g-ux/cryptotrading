...
跨文档引用统一用相对路径 + 锚点，例如 [`./02-data-model-and-migration.md#scheme-编码`](./02-data-model-and-migration.md#scheme-编码)。
...
| `ExitState` 字段：close+low+ma5+hold_days+…，**无 high、无 peak** | `strategy/exit_rules.py:53-71` |
...
| 主路径 `train_e2e`：_validate_params→base_scheme_codec→_step_labels | `worker/train_e2e_runner.py:455,557-575` |
