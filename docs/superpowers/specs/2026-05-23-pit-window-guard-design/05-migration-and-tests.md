...
**Migration A：factor_definitions.min_trade_days**
...
**Migration B：ml.jobs.warnings**

`apps/server/migrations/20260524_ml_jobs_warnings.sql` + 同名 `.ps1`：
...
详见 [06-warnings-and-startup.md §6.1](./06-warnings-and-startup.md#61-警告聚合写入-mljobswarnings)。两个 migration 互相独立，可任一顺序执行。
...
│                - 扩×2 仍不足 → warn skip                                  │
│                - trade_cal 未同步覆盖 → 单独 warn  test_trade_cal_        │
│                  trade_cal_not_synced，不归因为   not_synced.py          │
│                  factor_window_short                                     │
...
apps/server/migrations/
  ...
  20260524_ml_jobs_warnings.sql                                   (新, ~10 行)
  20260524_ml_jobs_warnings.ps1                                   (新, ~20 行)

apps/quant-pipeline/src/quant_pipeline/db/migrations/versions/
  20260525_0001_add_min_trade_days.py                             (新, ~50 行)
  20260525_0002_add_ml_jobs_warnings.py                           (新, ~20 行)
```
...
src/entities/ml/ml-job.entity.ts                      (改, +1 warnings 字段)
src/modules/quant/jobs/jobs.controller.ts             (改, GET :id 响应暴露 warnings)
src/modules/quant/jobs/jobs.service.ts                (改, 列表接口暴露 warnings_count)
src/modules/quant/factors/__tests__/                  (改 + 新)
src/modules/quant/jobs/__tests__/                     (改, warnings 字段)
```
