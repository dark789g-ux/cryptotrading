...
│    每缺口 [g0,g1] 尾部扩 padding(g1 后 30 交易日) 算 fwd/出场│
...
├─ ~ labels/runner.py:    compute_labels 接 force_recompute + 缺口循环(尾部padding)
...
| labels 增量 | 缺口循环+尾部padding+只写缺口 | gap_subranges | `compute_labels(...,force)` |
