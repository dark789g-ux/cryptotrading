...
```text
复现配置: base=KDJ_J<-10, exit=fixed_n(1), universe=全市场, 区间=2023-01~2026-05
...
- **容差与端点说明**：复现区间故意取与当初 DB 查询同口径（截至 2026-05），与扫描默认验证集的 2026-06 终点无关，勿混淆。`Kelly 绝对偏差 <0.005`（约占基线 3%）是按停牌/退市边界差几十笔的经验上限设的；**对齐后仍须人工核对 ret 分布与样本数，不能仅看 Kelly 落入区间就放行**——容差能容下的口径漂移并非都无害。
