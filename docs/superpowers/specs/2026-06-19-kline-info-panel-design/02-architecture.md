...
```ts
{
  ...
  trend?: string       // 可选，涨跌幅 CSS class：'trend-up' / 'trend-down' / ''。
                       // 直接取自 aSharesFormatters.trendClass() 返回值，颜色由全局 design-system.css 提供。
}
```

> **实现偏离说明**：spec 初稿写 `trend?: 'up' | 'down' | ''`（语义值，组件内映射成 CSS class）。实现时改为直接接受 CSS class 字符串，因为项目主力 `trendClass()` 函数返回的就是 `'trend-up'/'trend-down'/''`，若组件再做一次语义→class 映射纯属重复。调用方（如 UsStockInfoFields）直接传 `trendClass(row.pctChg)` 返回值即可。

渲染：`<div class="info-row"><span class="info-row__label">{{label}}</span><span class="info-row__value" :class="trend">{{value}}</span></div>`
