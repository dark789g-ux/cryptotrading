...
2. already_held：同策略在持同票的新信号 skip（reason 正确）；出场当日新信号可再进
   （先出场后开仓顺序的直接推论，显式断言）。
3. exposureCap 撞线（语义=严格大于才 skip，见 02）：数值用例——cap=0.33、当前敞口
   0.30、alloc=0.03 → 成交后恰好 0.33，**放行**；再一笔 alloc=0.03 → 0.36>0.33，
   **skip(exposure_cap)**。显式断言 `>` 而非 `>=`。
...
**前置不变量守护断言**（同 e2e 内）：官方双源（Q3-winner 与 Q1-winner）的
signal_date 集合无交集（SQL 一条即可）——这是 02"同 buy_date 单源前提"的命根，
从"声称"升级为"被守护"；若未来换源后断言失败，提示该场景下跨策略同日竞争真实
存在，引擎按 source 顺序处理的兜底路径开始生效。
...
（本期只交付本节文档，工具化按钮留二期。）影子期 picks（`regime_daily_pick`）只有
信号日快照、无前向收益。8 周后复核步骤：

1. 对影子期窗口的每个 trade 日 pick，按 config v1 的出场规则用 signal-stats
   **建方案+触发 run**，得到逐笔 trades。买入条件的构造**定死为 ts_code IN 列表**
   （picks 已快照具体标的，IN 列表精确复现当日清单，不受后续条件/数据漂移影响）；
   "复用原条件+日期窗"仅作为对照实验（验证条件稳定性）可选跑，不作为复核主口径。
