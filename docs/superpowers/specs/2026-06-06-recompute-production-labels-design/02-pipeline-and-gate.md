...
1. **`date_range.start` 恒传全区间起点 `20230103`** —— 月度推进时绝不传当月 1 号，否则 `g0_load` 夹不回上月、月初 MA=NaN、口径错（窗口依赖反向重现）。机制见 [04#阶段2 labels 重算](./04-recompute-and-cascade.md#阶段2-labels-重算)。
...
- 模型阶段默认"评估"，重训与 promote 由用户拍板（见 [04#阶段4 模型评估/重训](./04-recompute-and-cascade.md#阶段4-模型评估重训)）。
