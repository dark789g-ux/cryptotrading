...
```
holdings = {}  // date -> Map<symbol, {buy_date, buy_price, cost_remaining, name, sells_done[]}>
for t in trades:
    cost = t.buy_value
    ...
    // 该笔从 buy_date（含）到 last_sell_date（不含）每天都"在持仓"
    for d in trading_dates where buy_date <= d < last_sell_date:
        holdings[d].add(pos snapshot at d)
```

"snapshot at d" 计算：按 `t.sells` 中 `sell_date <= d` 的所有分批卖出，从 `cost_remaining` 中扣除对应比例。需要预先按 `sell_date` 升序处理 `sells[]`。
...
扫描 `daily.json` 按 date 升序，连续满足 `is_market_bull===1 && in_active_wave===1` 的天数视作一个段，记录 `[startDate, endDate]`。在 ECharts `markArea` 中渲染，颜色 `rgba(34,197,94,0.08)`。

### 5.4 净值基准化

策略净值序列 = `daily.capital_after_close / meta.params.initial_capital`。
基准序列 = `bench[i].value / bench[0].value`（bench 的字段名待实测，§7 验证步骤里包含"打开 bench.json 确认字段名"）。

### 5.5 "累计盈亏%" 字段口径（重要）

由于无逐股每日收盘价，**抽屉持仓表中"累计盈亏%"显示的不是当日 mark-to-market 浮盈**，而是：

```
该笔交易中，所有 sells[].sell_date <= 当前查看日期 的已实现盈亏 / buy_value
```

如果当前查看日期 ≤ buy_date 或该笔尚未发生任何 partial sell，显示 `+0.00%`。表头要加 tooltip 注明这点。
...
合并为单一实施任务，**一个 agent / 一次会话**就能做完，**不**适合 dispatching-parallel-agents 拆分。
...
- **bench.json 字段名未实测**：实施时先 `head bench.json`，按实际字段调整 §5.4 的取值代码。如果字段不是 `value`/`close` 之类，临时改名。
- **持仓快照算法对"同一标的多次买入"的处理**：`重复交易的股票`列表里有 4 次的（如 603036），同一日如果同时持有上次未平仓的同标的（实际数据扫了一遍没有这种情况）按"按 buy_date 分别记录"处理，不合并。
