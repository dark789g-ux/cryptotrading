...
Agent 应在此骨架基础上,根据用户策略需求替换 `entryConditions`/`exitMode`/`match` 等字段。字段值的选择依据见 §6 白名单。

**字段白名单详见**:[regime-backtest-field-whitelist.md](./regime-backtest-field-whitelist.md)
...
👉 **[regime-backtest-field-whitelist.md](./regime-backtest-field-whitelist.md)**
...
- 字段白名单是**易腐坏点**:每当 `strategy-conditions.types.ts` 的 `*_COL_MAP` 或 `rank-select.ts` 的 `RANK_FIELDS` 变更,必须同步更新 [regime-backtest-field-whitelist.md](./regime-backtest-field-whitelist.md)。
