...
8. **询问是否使用 subagent-driven-development** —— 用户批准 spec 后，询问是否基于该 spec 文档使用 `subagent-driven-development` 进行后续开发；如用户确认，则调用 `subagent-driven-development` 进行开发。
...
digraph brainstorming {
    ...
    "Ask: use subagent-driven-development?" [shape=diamond];
    ...
    "User reviews spec?" -> "Ask: use subagent-driven-development?" [label="approved"];
    "Ask: use subagent-driven-development?" -> "Invoke subagent-driven-development" [label="yes"];
    "Ask: use subagent-driven-development?" -> "End (user declines)" [label="no"];
}
```

**完成 spec 并经用户批准后，须询问是否基于该 spec 使用 `subagent-driven-development` 进行后续开发。** 未经用户确认前，不得调用 `subagent-driven-development` 或任何其它实现类 skill（如 frontend-design、mcp-builder 等）。
...
**询问是否使用 subagent-driven-development：**

用户批准 spec 后，必须明确询问是否基于该 spec 文档使用 `subagent-driven-development` 进行后续开发：

> "Spec 已通过。是否基于该 spec 使用 `subagent-driven-development` 拆分独立任务并并行开发？（是 / 否）"

- 如用户确认（"是"/"用"/"go" 等），立即调用 `subagent-driven-development` skill，并将 spec 路径作为输入传递，进入并行任务派发与执行流程。
- 如用户拒绝或希望另行处理，停在此处不再自动调用任何实现类 skill，由用户决定后续动作。
- 在用户明确回复之前，**不得**调用 `subagent-driven-development` 或任何其它实现类 skill。
