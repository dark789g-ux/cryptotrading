...
>
> **进度**：第一节（正确性 1/2/3）✅、第二节（轮询机制 4/5）✅、第三节（进度展示 6/7/8/9）✅、第四节（功能完整性 10）✅ 经决策不做 —— 全部完成（2026-07-08/09）。
...
## 四、功能完整性 ✅ 经决策不做（2026-07-09）

### 10. 命中结果看不到 ⚠️ → ✅ 经决策不做（2026-07-09）
- **现象（两处问题）**：
...
- **✅ 决策（2026-07-09）**：**整体不做。** 经用户确认，不需要命中明细展示，当前可接受。具体：
  - 不做抽屉 / 展开行明细展示（方案 A 不采纳）。
  - 不修 `SymbolsView` 的 `strategyId` 过滤（方案 B 不采纳）。
  - **保留项**：`getRunResult` API（前端）与后端 `GET :id/run/result` 路由维持原状，留作未来用；「查看 N 个命中结果」按钮跳转 `/symbols?strategyId=...` 的现状维持不变。
...
5. ~~**功能完整性**：10（命中结果展示，方案 A）~~ ✅ 经决策不做（2026-07-09），见第四节任务 10 决策说明。
