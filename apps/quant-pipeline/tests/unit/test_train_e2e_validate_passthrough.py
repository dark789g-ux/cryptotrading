"""train_e2e _validate_params 新参数严格校验单测（spec 04 §Python pytest）。
...
覆盖 spec 02 §严格校验表：
  ...
  - fwd_horizon_days / max_hold_days 的 scheme 匹配（不匹配 warn+忽略）
...
def test_lgb_multiclass_is_allowed_model() -> None:
    p = tr._validate_params(_valid(model="lgb-multiclass", label_scheme="dir3_band"))
    assert p.model == "lgb-multiclass"


# ---------------------------------------------------------------- 默认全 None
def test_all_new_params_default_to_none() -> None:
    ...
    assert p.fwd_horizon_days is None
    assert p.max_hold_days is None
...
# ---------------------------------------------------------------- fwd_horizon_days
def test_fwd_horizon_days_accepted_for_fwd_scheme() -> None:
    p = tr._validate_params(_valid(label_scheme="fwd_5d_ret", fwd_horizon_days=10))
    assert p.fwd_horizon_days == 10


def test_fwd_horizon_days_ignored_for_other_scheme(caplog: pytest.LogCaptureFixture) -> None:
    import logging

    with caplog.at_level(logging.WARNING):
        p = tr._validate_params(_valid(label_scheme="strategy-aware", fwd_horizon_days=10))
    assert p.fwd_horizon_days is None
    assert any("fwd_horizon_days_ignored" in r.message for r in caplog.records)


@pytest.mark.parametrize("v", [7, 0, 100])
def test_fwd_horizon_days_out_of_set_raises(v: int) -> None:
    with pytest.raises(ValueError, match="fwd_horizon_days"):
        tr._validate_params(_valid(label_scheme="fwd_5d_ret", fwd_horizon_days=v))


# ---------------------------------------------------------------- max_hold_days
def test_max_hold_days_accepted_for_strategy_aware() -> None:
    p = tr._validate_params(_valid(label_scheme="strategy-aware", max_hold_days=15))
    assert p.max_hold_days == 15


def test_max_hold_days_ignored_for_other_scheme(caplog: pytest.LogCaptureFixture) -> None:
    import logging

    with caplog.at_level(logging.WARNING):
        p = tr._validate_params(_valid(label_scheme="fwd_5d_ret", max_hold_days=15))
    assert p.max_hold_days is None
    assert any("max_hold_days_ignored" in r.message for r in caplog.records)


@pytest.mark.parametrize("v", [5, 40])
def test_max_hold_days_out_of_range_raises(v: int) -> None:
    with pytest.raises(ValueError, match="max_hold_days"):
        tr._validate_params(_valid(label_scheme="strategy-aware", max_hold_days=v))
