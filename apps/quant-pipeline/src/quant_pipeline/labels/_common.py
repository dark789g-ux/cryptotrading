...
def apply_hfq(df: pd.DataFrame) -> pd.DataFrame:
    """注入后复权列 close_adj / low_adj / high_adj / open_adj。

    纯后复权：close_adj = close × adj_factor、low_adj = low × adj_factor、
    high_adj = high × adj_factor、open_adj = open × adj_factor（逐行）。只依赖各
    交易日当日已知的 adj_factor、不除任何窗口基准 → 绝对水平 PIT 安全。
    ...
    low_adj / high_adj / open_adj 仅在源 df 含对应原始列时注入（对称处理）；
    band_lock scheme 买在 T+1 开盘 → 需 open_adj（其它 scheme 不读该列，无影响）。
    adj_factor 为 NULL 的行 → close_adj/low_adj/high_adj/open_adj 为 NaN；统计并 warn。
    """
