...
单一真理源）。所有指标基于三分类 {down=0, flat=1, up=2}：
  - accuracy / macro_f1 / per_class(precision/recall/f1/support)
  - confusion_matrix：行=真实类、列=预测类，顺序固定 [down, flat, up]
  - 排序代理指标 ic / rank_ic：按 score = P(涨) − P(跌) 与**真实次日后复权收益** r 算
    （口径同 labels/direction_3class）。NaN（含 ±inf）样本剔除后再算相关性，
    有效样本不足 2 个 → NaN（不静默填 0 / 序数）。

纯 numpy（不引 sklearn / torch），便于无 torch/lightgbm 环境跑通结构单测。
"""
...
def score_ic_rank_ic(score: np.ndarray, true_ret: np.ndarray) -> tuple[float, float]:
    """排序分 score = P(涨) − P(跌) 与**真实次日后复权收益**的 IC / RankIC。

    true_ret = close_adj(t+1)/close_adj(t)−1（口径同 labels/direction_3class）。
    取不到 t+1 收益的样本（停牌 / 退市 / 末日 / 缺数）由调用方填 NaN。

    NaN（含 ±inf）生成 mask，score 与 true_ret 同步剔除；有效样本 < 2 → NaN。
    """
...
def build_oos_metrics(
    *,
    ...
    """组装三分类 oos_metrics JSON 结构（task=classification_3class）。

    lstm 与 lgb-multiclass 共用，保证前端统一展示口径一致。
    """
