...
.status-icon.idle { background: rgba(148,163,184,.2); color: #94a3b8; }
.status-icon.running { background: rgba(59,130,246,.2); color: #3b82f6; }
.status-icon.done { background: rgba(16,185,129,.2); color: #10b981; }
.status-icon.error { background: rgba(239,68,68,.2); color: #ef4444; }
.spinning { animation: spin 1s linear infinite; }
@keyframes spin { from { transform: rotate(0deg); } to { transform: rotate(360deg); } }
.status-info { text-align: center; margin-bottom: 20px; }
.status-info h3 { margin: 0 0 8px; font-size: 18px; color: var(--text-primary); }
.status-message { margin: 0; color: var(--text-secondary); font-size: 14px; }
.sync-progress { width: 100%; }
.overview-grid { display: grid; grid-template-columns: repeat(3, 1fr); gap: 16px; }
.overview-item { text-align: center; padding: 16px; background: var(--glass-bg); border-radius: 12px; border: 1px solid var(--glass-border); }
.overview-value { font-size: 18px; font-weight: 600; color: var(--text-primary); margin-bottom: 4px; }
.overview-label { font-size: 12px; color: var(--text-secondary); }
.log-container { height: 200px; overflow-y: auto; background: var(--glass-bg); border-radius: 12px; padding: 12px; font-family: 'Courier New', monospace; font-size: 13px; }
.log-empty { text-align: center; color: var(--text-muted); padding: 40px; }
.log-item { padding: 4px 0; border-bottom: 1px solid var(--glass-border); }
