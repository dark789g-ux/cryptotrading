...
export function useUsStocksQuery(message: {
  error: (content: string) => void
  success?: (content: string) => void
}) {
  const loading = ref(false)
  const rows = ref<UsStockRow[]>([])
  const total = ref(0)
  const page = ref(1)
  const pageSize = ref(10)
  const searchQuery = ref('')
  const selectedTheme = ref<string | null>(null)
  const selectedStockType = ref<string | null>(null)
  const priceMode = ref<UsStockPriceMode>('qfq')
  const pctChangeMin = ref<number | null>(null)
  const advancedConditions = ref<Condition[]>([])
  const sortKey = ref<string | null>(null)
  const sortOrder = ref<'ascend' | 'descend' | null>(null)
  const summary = ref<UsStockSummary>({
    totalSymbols: '0',
  ...
  const stockTypeOptions = ref<SelectOption[]>([])
  ...
  const summaryItems = computed<SummaryItem[]>(() => [
    { label: '标的总数', value: summary.value.totalSymbols, note: '已跟踪美股', className: '' },
  ...
  async function loadData() {
    loading.value = true
    try {
      const res = await usStocksApi.query({
        page: page.value,
        pageSize: pageSize.value,
        q: searchQuery.value,
        ...
        sort: { field: sortKey.value ?? 'ticker', order: sortOrder.value },
  ...
  async function loadSummary() {
    try {
      summary.value = await usStocksApi.getSummary()
    } catch {
  ...
  async function loadFilterOptions() {
    try {
      const options = await usStocksApi.getFilterOptions()
      themeOptions.value = options.themes.map((item) => ({ label: item.value, value: item.value }))
      stockTypeOptions.value = options.stockTypes.map((item) => ({ label: item.value, value: item.value }))
    } catch {
      themeOptions.value = []
